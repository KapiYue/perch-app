...
final class ClipboardWatcher {
    ...
    private func tick() {
        ...
        // 内容一个字节都不读、更不落盘。
        //
        // 🔴 **这条没有开关，永远生效**（2026-08-25 定的口径）。
        // 早先它是设置页里一个默认关闭的开关，理由是「可靠性取决于来源 App 规不规范打标」——
        // 但那句话论证的是**不能只靠它**，不是「标记摆在那儿也要装作没看见」。
        // 来源 App 已经明说了「这条别记」，尊重它不需要用户再授权一次；
        // 而设置页少一行开关加一坨脚注，用户要读的东西也少了一半。
        if carriesSkipMarker(pasteboard) {
            pendingReads = 0
            pendingSourceBundleID = nil
            return
        }
