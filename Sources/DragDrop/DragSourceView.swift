...
struct ClickContext {
    ...
    /// 第几击。**2 = 双击**：剪贴板区切换固定，文件区用默认应用打开（M4）。
    ///
...
extension View {
    ...
    func dragOut(_ items: [PerchItem], onClick: (@MainActor (ClickContext) -> Void)? = nil) -> some View {
        overlay(DragOutSurface(items: items, onClick: onClick))
    }
    ...
    func clickAction(_ action: @escaping @MainActor () -> Void) -> some View {
        overlay(ClickSurface(action: action))
    }
}
